...
public class ContentAction implements CommandAction {// 글내용 처리
	...
	public String requestPro(HttpServletRequest request, HttpServletResponse response) throws Throwable {
		...
		int bno = Integer.parseInt(request.getParameter("b_no"));// 해당 글번호
		String pageNum = request.getParameter("pageNum");// 해당 페이지 번호

		BoardDBBean dbPro = BoardDBBean.getInstance();// DB처리
		BoardDataBean article = dbPro.getArticle(bno);// 해당 글번호에 대한 해당 레코드

		// 해당 뷰에서 사용할 속성
		request.setAttribute("b_no", new Integer(bno));
